...
| FR-2.9 | Add documents to an existing patient from the dashboard | P0 |
...
| 4. Cross-check | LLM | Interactions, allergy and warning conflicts, severity, bilingual explanation. Gated by temporal overlap: a pair is only considered when the two prescriptions' active windows meet |
...
**Temporal overlap gate (stages 3 and 4).** Each medication carries an active window of
[prescribed date, prescribed date + duration]. Where the document printed no duration, the window is
open-ended only if the instructions say the drug continues ("as and when required", "continue",
a maintenance dose); otherwise a conservative 30-day course is assumed rather than an indefinite one.
Two medications are cross-checked against each other only when their windows meet. Two rows from one
document are always checked, whatever their dates — the same-document contradiction (FR-5.5) does not
depend on date reasoning. Where either date could not be read, the pair is still checked and the
finding says so, rather than being silently dropped or silently treated as concurrent. The same gate
scopes the therapeutic-class duplicate check, which claims the patient may be taking two of a class
at once.
...
| Cross-checking is scoped to temporally overlapping prescriptions, not the full patient history | Flagging non-concurrent drugs as interacting would be clinically misleading — and the volume of cross-year pairs buries the genuine same-visit findings. Where a date is unreadable the pair is kept and labelled, never dropped |
